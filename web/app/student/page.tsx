...
export default function StudentPortalPage() {
  useEffect(() => {
    api<Me>("/api/auth/me")
      .then((me) => {
        if (me.role === "STUDENT") window.location.replace(homePath("STUDENT"));
      })
      .catch(() => undefined);
  }, []);
    ...
    <div className="hero-grid flex min-h-full flex-col">
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-5 pb-16">
        <section className="card portal-card">
          <h1 className="text-2xl font-semibold text-[var(--navy)]">Student Portal</h1>
          <p className="mt-2 text-sm text-[var(--muted)]">Login with your matricule, or register.</p>
